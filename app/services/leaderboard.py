...
class LeaderboardService:
    ...
    def get_global_leaderboard_for_user(self, user: User) -> LeaderboardResponse:
        leaderboard_user = self._uow.leaderboard_users.find_by_user(user.id)
        days_until_end = days_until_next_sunday()
        league = leaderboard_user.league if leaderboard_user else League.BRONZE
        xp = leaderboard_user.xp if leaderboard_user else 0
        ...
        return LeaderboardResponse(
            ...
            current=[LeaderboardEntry(rank=0, username=user.email, xp=xp)] # Will add proper rank and users around the user in the future using redis
        )
