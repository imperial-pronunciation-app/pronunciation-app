...
# async def test_update_xp_existing_record(test_user: User, uow: UnitOfWork) -> None:
#     # Pre
...
#     # When
#     leaderboard_user = await user_service.update_xp(test_user, xp_gain)
...
# async def test_reset_leaderboard(test_user: User, auth_client: TestClient, uow: UnitOfWork) -> None:
#     # Pre
#     initial_xp = 10
#     user_service = UserService(uow)
#     await user_service.update_xp(test_user, initial_xp)
#     assert test_user.leaderboard_entry.xp == initial_xp

#     # When
#     leaderboard_service = LeaderboardService(uow)
#     await leaderboard_service.reset_leaderboard()
#     response = auth_client.get("/api/v1/leaderboard/global")

#     # Then
#     expected_xp = int(math.log2(initial_xp + 1))
#     assert (await uow.leaderboard_users.get_by_user(test_user.id)).xp == expected_xp
...
# async def test_promotion_demotion(uow: UnitOfWork, sample_leaderboard_users_silver: Sequence[LeaderboardUserLink]) -> None:    
...
#     # Then
#     leaderboard_users = await uow.leaderboard_users.all()
