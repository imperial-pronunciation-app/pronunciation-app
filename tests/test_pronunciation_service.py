...
@pytest.mark.parametrize(
    "pronounced_phonemes, pronounced_words, expected_alignment, min_score, max_score",
    [
        # Perfect match (100%)
        (["t", "e", "s", "t"],
         ["test"],
...
def test_match_pronunciation(make_word: WordFactory, make_phoneme: PhonemeFactory, pronounced_phonemes: List[str], pronounced_words: List[str], expected_alignment: List[Tuple[Optional[PhonemePublic], Optional[PhonemePublic]]], min_score: int, max_score: int, uow: UnitOfWork) -> None:
    ...
    alignment, score = PronunciationService(uow).evaluate_pronunciation(word, pronounced_phonemes, pronounced_words)
